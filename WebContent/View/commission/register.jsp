...
<% Collection<ActivityTutorBean> activitiesCollection = (Collection<ActivityTutorBean>) request.getAttribute("activitiesCollection");  
    ...
    if (activitiesCollection == null){
		response.sendRedirect(response.encodeRedirectURL("/TutoratoSmart/ShowRegisterTutoringSupervision"));
		return;
	}
					...
					<% 	Iterator<?> it = activitiesCollection.iterator();
					 	for(int i = 0 ; i < activitiesCollection.size() && it.hasNext(); i++) { 
				 			...
				 			<tr>
					 			...
					 			<td><%= bean.getState() %></td>
					 			<td><a href ="<%= response.encodeURL(request.getContextPath() + "/ShowActivityTutoringSupervision?id=" + bean.getIdActivity()) %>">Mostra</a></td>
					 		</tr>		 
					<%	} 
				} %>			
			</table>
